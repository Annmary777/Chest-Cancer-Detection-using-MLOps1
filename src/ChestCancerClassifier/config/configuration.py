...
class ConfigurationManager:
    ...
    def get_training_config(self) -> TrainingConfig:
        ...
        training_data = os.path.join(
            self.config["data_ingestion"]["unzip_dir"], "fall and nonfall"
        )
        create_directories([Path(training["root_dir"])])

        # Set epochs to 1 for testing purposes
        training_config = TrainingConfig(
            root_dir=Path(training["root_dir"]),
            trained_model_path=Path(training["trained_model_path"]),
            updated_base_model_path=Path(prepare_base_model["updated_base_model_path"]),
            training_data=Path(training_data),
            params_epochs=1,  # Overriding to 1 epoch
    ...
    def get_evaluation_config(self) -> EvaluationConfig:
        eval_config = EvaluationConfig(
            ...
            mlflow_uri="https://dagshub.com/Annmary777/Chest-Cancer-Detection-using-MLOps1.mlflow",
